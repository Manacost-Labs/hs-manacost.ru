...
test('pending is owner-only; public author links and paid title are server-derived', async t => {
  ...
  assert.equal((await (await f.call('/reader-api/v1/threads/17/comments')).json()).items.length, 0);
  assert.equal((await (await f.call('/reader-api/v1/threads/17/comments', { headers: other.headers })).json()).items.length, 0);
  assert.equal((await (await f.call('/reader-api/v1/threads/17/comments', { headers: user.headers })).json()).items[0].status, 'pending');
  assert.equal((await f.call(`/reader-api/v1/readers/${user.me.profile.id}`)).status, 404);
  f.comments.review(comment.id, { version: 1, decision: 'publish', actor: 'synthetic-moderator' });
