...
description: >-
  ...
  (use the project's test-driven-development route), for frontend component or
...
- Respect robots.txt, terms, authentication boundaries, and rate limits; bound the scrape by page count and delay. Stop when an anti-bot challenge or access control blocks the run; do not bypass it.
...
- **Test strategy, framework selection, or QA process** — route to the project test-driven-development skills.
- **Frontend component/state/architecture design or implementation guidance** — route to `agent-frontend-ui-engineering`.
- **Anti-bot, WAF, authentication, or rate-limit bypass** — stop and obtain authorized access; never evade the control.
- **Load/performance testing at scale** (k6, Locust, Gatling, JMeter) — use the project performance route and an explicitly approved load-test scope.
