...
try {
  ...
  assert.equal(await zoomPage.getByRole('button', { name: 'Отправить комментарий' }).isVisible(), true);
  ...
  failPost=true; await page.getByRole('checkbox', {name:/Согласен/}).check(); await page.getByLabel('Комментарий').fill('Новый ответ'); await page.getByRole('button', {name:'Отправить комментарий'}).click(); await page.getByRole('button',{name:'Повторить отправку'}).waitFor(); assert.equal(await page.getByLabel('Комментарий').isDisabled(),true); const original=writes.at(-1); failPost=false; await Promise.all([page.waitForResponse(response => response.request().method() === 'POST' && response.status() === 201), page.getByRole('button',{name:'Повторить отправку'}).click()]);
  await page.getByText('Комментарий ожидает проверки.').waitFor(); assert.ok(writes.length >= 2); assert.ok(writes.every(write=>JSON.stringify(write)===JSON.stringify(original))); assert.equal(writes[0].publicConsent, true); assert.match(writes[0].operationId, /^[0-9a-f-]{36}$/i);
  await page.getByRole('button', {name:'Ответить'}).click(); await page.getByLabel('Комментарий').fill('Реплика'); assert.equal(await page.getByRole('button', {name:'Отменить ответ'}).isVisible(), true);
