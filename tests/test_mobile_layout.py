...
class MobileLayoutTest(unittest.TestCase):
    ...
    def test_enqueues_scoped_public_mobile_layout(self) -> None:
        ...
        self.assertEqual("1.0.2", result["styles"]["manacost-mobile-layout"][2])
    ...
    def test_mobile_css_replaces_the_article_cover_with_a_plain_text_header(self) -> None:
        ...
        self.assertGreaterEqual(css.count("max-inline-size: 300px"), 2)
        self.assertIn("font-weight: 600", css)
        self.assertIn("text-align: center", css)
        self.assertIn(".entry-title::after", css)
        self.assertIn("inline-size: 40px", css)
        self.assertIn("background: var(--td_theme_color, #2a77bf)", css)
        self.assertIn("font-size: 15px", css)
        self.assertIn("max-inline-size: 300px", css)
