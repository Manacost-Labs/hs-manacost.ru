...
function hs_reader_comments_shell(): string {
	...
	return '<section id="reader-comments" class="mc-reader-ui mc-comments" data-mc-comments data-post-id="' . esc_attr( (string) $post_id ) . '" aria-labelledby="reader-comments-title">'
		...
		. '<div class="mc-comments__composer-heading"><h3>' . esc_html__( 'Написать комментарий', 'hs-manacost-reader' ) . '</h3></div><p data-comments-reply hidden></p><button class="mc-ui-button mc-ui-button--text" type="button" data-comments-cancel hidden>' . esc_html__( 'Отменить ответ', 'hs-manacost-reader' ) . '</button>'
		. '<label class="mc-ui-label" for="reader-comment-body">' . esc_html__( 'Комментарий', 'hs-manacost-reader' ) . '</label><textarea class="mc-ui-control" id="reader-comment-body" data-comments-body rows="5" minlength="2" maxlength="1000" required></textarea>'
		. '<label class="mc-comments__consent"><input type="checkbox" data-comments-consent required> ' . esc_html__( 'Согласен(на) на немедленную публикацию: имени, аватара, профиля, любимого класса и описания. При подтверждённой оплате будет указан титул «Платный подписчик».', 'hs-manacost-reader' ) . '</label>'
		. '<div class="mc-comments__composer-footer"><span class="mc-ui-help">' . esc_html__( 'До 1000 символов.', 'hs-manacost-reader' ) . '</span><button class="mc-ui-button" type="submit" data-comments-submit>' . esc_html__( 'Опубликовать', 'hs-manacost-reader' ) . '</button></div></form>'
